...
static void gxmicro_qc_prep(struct ata_queued_cmd *qc)
{
...
static unsigned int gxmicro_issue_cmd_rw(struct ata_queued_cmd *qc)
{
	...
	desc = dmaengine_prep_slave_sg(gchan, qc->sg, qc->n_elem, qc->dma_dir, 0);
	if (!desc) {
